...
#ARCHIVE
mkdir ${ARCHIVE_DIR}
/usr/bin/xcodebuild -workspace ${XCODE_WORKSPACE_FILE} -configuration Release -scheme ${BUILD_SCHEME} -archivePath ${ARCHIVE_DIR}/${ARCHIVE_FILE_NAME} archive GOOGLE_APP_ID=${GOOGLE_APP_ID} PROVISIONING_PROFILE_SPECIFIER=${PROVISIONING_PROFILE_NAME}