...
#ARCHIVE
mkdir ${ARCHIVE_DIR}
/usr/bin/xcodebuild -workspace ${XCODE_WORKSPACE_FILE} -configuration Release -scheme ${BUILD_SCHEME} build GOOGLE_APP_ID=${GOOGLE_APP_ID} PRODUCT_BUNDLE_IDENTIFIER=${BUNDLE_IDENTIFIER} PROVISIONING_PROFILE_SPECIFIER=${PROVISIONING_PROFILE_FILENAME}